...
async def post_tweet(
    ...
    """
    snap = await snaps_dao.create_snaps_model(user_id=incoming_message.user_id, _content=incoming_message.content)
    insp = inspect(snap)
...
async def get_tweet(
    user_id: int, tweet_id: int, snaps_dao: SnapDAO = Depends()) -> None:
    """
    Gets a tweet
    """
...
async def get_tweets(
    user_id: int, snaps_dao: SnapDAO = Depends()) -> FeedPack:
    """
